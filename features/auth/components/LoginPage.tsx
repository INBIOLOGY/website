...
import { authenticate } from '@/features/auth/client'
...
// LOGIN PAGE
// ─────────────────────────────────────────────────────────────────────────────
export function LoginPage({ onLogin, setPage, addToast }) {
  ...
  const handle = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await authenticate(mode as 'login' | 'register', {
        email: form.email,
  ...
  const inpStyle: CSSProperties = { width: '100%', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '10px 14px', fontSize: '13px', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
