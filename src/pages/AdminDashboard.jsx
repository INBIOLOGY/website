...
import {
  BarChart2, BookOpen, Video, FileSpreadsheet, Award, Image, Users,
  Edit, Trash2
} from 'lucide-react';
...
export default function AdminDashboard({ courses, setCourses, slides, setSlides, coupons, setCoupons, orders, setOrders, addToast }) {
    ...
    <div className="admin-container" style={{ background: C.page, paddingTop: '70px' }}>
      ...
      <main className="admin-main" style={{ flex: 1, marginLeft: '220px', padding: '24px', maxWidth: 'calc(100% - 220px)' }}>
        <div style={{ maxWidth: '950px' }}>
          ...
          {tab === 'students' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <h2 style={{ fontSize: '18px', fontWeight: 950, color: C.navy, margin: 0 }}>รายชื่อนักเรียนในระบบ</h2>
              <div style={{ background: 'white', border: '1px solid #E5E7EB', borderRadius: '20px', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                  <thead style={{ background: '#F9FAFB', borderBottom: '1px solid #E5E7EB' }}>
                    <tr>
                      {['ชื่อนักเรียน', 'อีเมล', 'โรงเรียน', 'ระดับชั้น', 'คอร์สที่ลงทะเบียน'].map(h => (
                        <th key={h} style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 800, color: '#6B7280' }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[
                      ...
                      <tr key={st.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                        ...
                        <td style={{ padding: '12px 16px', color: '#6B7280' }}>{st.email}</td>
                        <td style={{ padding: '12px 16px' }}>{st.school}</td>
                        <td style={{ padding: '12px 16px', fontWeight: 700 }}>{st.level}</td>
                        <td style={{ padding: '12px 16px', fontWeight: 850, color: C.sky }}>{st.enrolledCount} คอร์ส</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {/* Modal Course Edit / Add */}
          <Modal isOpen={courseModal} onClose={() => setCourseModal(false)} title={editCourse ? 'แก้ไขคอร์สเรียน' : 'เพิ่มคอร์สใหม่'}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div>
                <label style={labelStyle}>ชื่อคอร์สเรียน</label>
                <input type="text" placeholder="เช่น Bio Intensive I" value={courseForm.title} onChange={e => setCF(p => ({ ...p, title: e.target.value }))} style={inpStyle} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div>
                  <label style={labelStyle}>ราคาโปรโมชัน (บาท)</label>
                  <input type="number" placeholder="1290" value={courseForm.price} onChange={e => setCF(p => ({ ...p, price: e.target.value }))} style={inpStyle} />
                </div>
                <div>
                  <label style={labelStyle}>ราคาเต็ม (บาท)</label>
                  <input type="number" placeholder="2500" value={courseForm.originalPrice} onChange={e => setCF(p => ({ ...p, originalPrice: e.target.value }))} style={inpStyle} />
                </div>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                <div>
                  <label style={labelStyle}>จำนวนชั่วโมงเรียน</label>
                  <input type="number" placeholder="30" value={courseForm.hours} onChange={e => setCF(p => ({ ...p, hours: e.target.value }))} style={inpStyle} />
                </div>
                <div>
                  <label style={labelStyle}>ระดับชั้น</label>
                  <select value={courseForm.Level} onChange={e => setCF(p => ({ ...p, Level: e.target.value }))} style={{ ...inpStyle, background: 'white' }}>
                    {['ม.4', 'ม.5', 'ม.6', 'ติวสอบ'].map(l => <option key={l} value={l}>{l}</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label style={labelStyle}>รายละเอียดคอร์ส</label>
                <textarea rows={3} placeholder="คำอธิบายคอร์ส..." value={courseForm.description} onChange={e => setCF(p => ({ ...p, description: e.target.value }))} style={{ ...inpStyle, resize: 'none' }} />
              </div>
              <button onClick={saveCourse} style={{ marginTop: '10px', width: '100%', background: C.sky, color: 'white', fontWeight: 800, fontSize: '13px', padding: '12px', borderRadius: '10px', border: 'none', cursor: 'pointer' }}>
                บันทึกข้อมูลคอร์สเรียน
              </button>
            </div>
          </Modal>
